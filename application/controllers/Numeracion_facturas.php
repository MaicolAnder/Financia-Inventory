...
class Numeracion_facturas extends CI_Controller
{
    ...
    {
        $data['page'] = 'Numeración de facturas de venta';
    ...
    {
        $facs = $this->Numeracion_facturas_model->get_all('*', [], ['Id_NumFac'=>'DESC'], [1]);
        if ($facs) {
            $Numero_NumFac = $facs[0]->Numero_NumFac+1;
            $Nombre_NumFac = '';
            $Defecto_NumFac = 'Inactivo';
        } else {
            $Numero_NumFac = 1;
            $Nombre_NumFac = 'Principal';
            $Defecto_NumFac = 'Activo';
        }
        $data = array(
            'button' => 'Guardar',
            'action' => site_url('numeracion_facturas/create_action'),
		    'Id_NumFac' => set_value('Id_NumFac'),
		    'Nombre_NumFac' => $Nombre_NumFac,
		    'Prefijo_NumFac' => set_value('Prefijo_NumFac','FV'),
		    'Numero_NumFac' => $Numero_NumFac,
		    'Resolucion_NumFac' => set_value('Resolucion_NumFac'),
		    'Activo_NumFac' => set_value('Activo_NumFac','Activo'),
		    'Defecto_NumFac' => $Defecto_NumFac,
		    'Primary_Usu' => set_value('Primary_Usu'),
		);
		   
        $data['page'] = 'Nueva numeración';
        $data['module']= $this->module;
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            if ($row->Activo_NumFac=='Activo') {
                $Activo_NumFac = 'Inactivo';
            } else {
                $Activo_NumFac = 'Activo';
            }
            $this->Numeracion_facturas_model->update($id,['Activo_NumFac'=>$Activo_NumFac]);
            $this->session->set_flashdata('message', 'Registro '.$Activo_NumFac.' exitosamente');
            redirect(site_url('numeracion_facturas'));
        } else {
            $this->session->set_flashdata('message', 'Registro no encontrado');
            redirect(site_url('numeracion_facturas'));
