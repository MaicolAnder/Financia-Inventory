...
class Roles_permiso_model extends CI_Model
{

    public $table = 'roles_permiso';
    public $id = 'Id_RolPerm';
    public $order = 'DESC';
    ...
    function json(
        ...
        $links=[],
        $view = 'roles_permiso',
        $elements = 'Id_RolPerm,Id_Rol,Id_Perm,Editar,Crear,Ver,Listar,Primary_Usu', 
        $find_id = 'Id_RolPerm',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'roles_permiso.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('roles_permiso/read/$1'),'Read')." | ".anchor(site_url('roles_permiso/update/$1'),'Update')." | ".anchor(site_url('roles_permiso/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('roles_permiso/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('roles_permiso/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('roles_permiso/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_RolPerm', $q);
			$this->db->or_like('Editar', $q);
			$this->db->or_like('Crear', $q);
			$this->db->or_like('Ver', $q);
			$this->db->or_like('Listar', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        $this->db->join('permiso', $this->table.'.Id_Perm = permiso.Id_Perm', $join_type);
    ...
    // funciones anidada para la tabla permiso 
    function get_foreing_Id_Perm($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('permiso.*,'.$this->table.'.*');
            $this->db->join('permiso', $this->table.'.Id_Perm = permiso.Id_Perm', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* End of file Roles_permiso_model.php */
/* Location: ./application/models/Roles_permiso_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-10 01:50:22 */
/* www.margunsoft.com */