...
class Mensajes_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'mensajes',
        $elements = 'Id_Men,Asunto_Men,Mensaje_Men,FechaRegistro_Men,FechaVisto_Men,DestinatarioEmail_Men,Estado_Men,Masivo_Men,Id_MenTip,Id_Per,Primary_Usu', 
        $find_id = 'Id_Men',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'mensajes.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('mensajes/read/$1'),'Read')." | ".anchor(site_url('mensajes/update/$1'),'Update')." | ".anchor(site_url('mensajes/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('mensajes/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('mensajes/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('mensajes/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_Men', $q);
			$this->db->or_like('Asunto_Men', $q);
			$this->db->or_like('Mensaje_Men', $q);
			$this->db->or_like('FechaRegistro_Men', $q);
			$this->db->or_like('FechaVisto_Men', $q);
			$this->db->or_like('DestinatarioEmail_Men', $q);
			$this->db->or_like('Estado_Men', $q);
			$this->db->or_like('Masivo_Men', $q);
			$this->db->or_like('Id_MenTip', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('mensaje_tipo.*');
            $this->db->select('persona.*');
		} else{
            $this->db->select($elements);
        }
        $this->db->join('mensaje_tipo', $this->table.'.Id_MenTip = mensaje_tipo.Id_MenTip', $join_type);
    ...
    // funciones anidada para la tabla mensaje_tipo 
    function get_foreing_Id_MenTip($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('mensaje_tipo.*,'.$this->table.'.*');
            $this->db->join('mensaje_tipo', $this->table.'.Id_MenTip = mensaje_tipo.Id_MenTip', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* Location: ./application/models/Mensajes_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-10 01:50:09 */
/* www.margunsoft.com */