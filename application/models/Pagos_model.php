...
class Pagos_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'pagos',
        $elements = 'Id_Pag,Valor_Pag,Fecha_Pag,Id_MetPag,Id_Tran,Primary_Usu', 
        $find_id = 'Id_Pag',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'pagos.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('pagos/read/$1'),'Read')." | ".anchor(site_url('pagos/update/$1'),'Update')." | ".anchor(site_url('pagos/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('pagos/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('pagos/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('pagos/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_Pag', $q);
			$this->db->or_like('Valor_Pag', $q);
			$this->db->or_like('Fecha_Pag', $q);
			$this->db->or_like('Id_MetPag', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('metodo_pago.*');
            $this->db->select('transacciones.*');
		} else{
            $this->db->select($elements);
        }
        $this->db->join('metodo_pago', $this->table.'.Id_MetPag = metodo_pago.Id_MetPag', $join_type);
    ...
    // funciones anidada para la tabla metodo_pago 
    function get_foreing_Id_MetPag($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('metodo_pago.*,'.$this->table.'.*');
            $this->db->join('metodo_pago', $this->table.'.Id_MetPag = metodo_pago.Id_MetPag', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-10 01:50:13 */
/* www.margunsoft.com */