...
class Metodo_pago_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'metodo_pago',
        $elements = 'Id_MetPag,Nombre_MetPag,Estado_MePag,FechaRegistro,Primary_Usu', 
        $find_id = 'Id_MetPag',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'metodo_pago.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('metodo_pago/read/$1'),'Read')." | ".anchor(site_url('metodo_pago/update/$1'),'Update')." | ".anchor(site_url('metodo_pago/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('metodo_pago/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('metodo_pago/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('metodo_pago/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_MetPag', $q);
			$this->db->or_like('Nombre_MetPag', $q);
			$this->db->or_like('Estado_MePag', $q);
...
/* End of file Metodo_pago_model.php */
/* Location: ./application/models/Metodo_pago_model.php */
