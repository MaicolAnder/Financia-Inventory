...
class Documento_observaciones_model extends CI_Model
{

    public $table = 'documento_observaciones';
    public $id = 'Id_DocObs';
    public $order = 'DESC';
    ...
    function json(
        $search_fields=[],
        $links=[],
        $view = 'documento_observaciones',
        $elements = 'Id_DocObs,Nombre_DocObs,Id_DocEst,Id_Usu,FechaRegistro,Id_Doc', 
        $find_id = 'Id_DocObs',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'documento_observaciones.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('documento_observaciones/read/$1'),'Read')." | ".anchor(site_url('documento_observaciones/update/$1'),'Update')." | ".anchor(site_url('documento_observaciones/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('documento_observaciones/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('documento_observaciones/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('documento_observaciones/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
        if ($q != NULL) {
			$this->db->like('Id_DocObs', $q);
			$this->db->or_like('Nombre_DocObs', $q);
			$this->db->or_like('Id_DocEst', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('documento_estado.*');
		} else{
            $this->db->select($elements);
        }
        ...
        $this->db->join('documento_estado', $this->table.'.Id_DocEst = documento_estado.Id_DocEst', $join_type);
    ...
    // funciones anidada para la tabla documento_estado 
    function get_foreing_Id_DocEst($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('documento_estado.*,'.$this->table.'.*');
            $this->db->join('documento_estado', $this->table.'.Id_DocEst = documento_estado.Id_DocEst', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* End of file Documento_observaciones_model.php */
/* Location: ./application/models/Documento_observaciones_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-11 19:37:02 */
/* www.margunsoft.com */