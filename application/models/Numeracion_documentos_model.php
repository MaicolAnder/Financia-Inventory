...
class Numeracion_documentos_model extends CI_Model
{

    public $table = 'numeracion_documentos';
    public $id = 'Id_NumDoc';
    public $order = 'DESC';
    ...
    function json(
        $search_fields=[],
        $links=[],
        $view = 'numeracion_documentos',
        $elements = 'Id_NumDoc,Inicial_NumDoc,Siguiente_NumDoc,Id_DocTip,Id_TranTip,Primary_Usu', 
        $find_id = 'Id_NumDoc',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'numeracion_documentos.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('numeracion_documentos/read/$1'),'Read')." | ".anchor(site_url('numeracion_documentos/update/$1'),'Update')." | ".anchor(site_url('numeracion_documentos/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('numeracion_documentos/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('numeracion_documentos/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('numeracion_documentos/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_NumDoc', $q);
			$this->db->or_like('Inicial_NumDoc', $q);
			$this->db->or_like('Siguiente_NumDoc', $q);
			$this->db->or_like('Id_DocTip', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('documento_tipo.*');
            $this->db->select('transaccion_tipo.*');
		} else{
            $this->db->select($elements);
        }
        $this->db->join('documento_tipo', $this->table.'.Id_DocTip = documento_tipo.Id_DocTip', $join_type);
    ...
    // funciones anidada para la tabla documento_tipo 
    function get_foreing_Id_DocTip($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('documento_tipo.*,'.$this->table.'.*');
            $this->db->join('documento_tipo', $this->table.'.Id_DocTip = documento_tipo.Id_DocTip', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* End of file Numeracion_documentos_model.php */
/* Location: ./application/models/Numeracion_documentos_model.php */
