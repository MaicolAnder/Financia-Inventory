...
class Municipio_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'municipio',
        $elements = 'Id_Mun,Nombre_Num,Codigo_Mun,Id_Dep', 
        $find_id = 'Id_Mun',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'municipio.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('municipio/read/$1'),'Read')." | ".anchor(site_url('municipio/update/$1'),'Update')." | ".anchor(site_url('municipio/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('municipio/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('municipio/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('municipio/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
        if ($q != NULL) {
			$this->db->like('Id_Mun', $q);
			$this->db->or_like('Nombre_Num', $q);
			$this->db->or_like('Codigo_Mun', $q);
			$this->db->or_like('Id_Dep', $q);
		}
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        $this->db->join('departamento', $this->table.'.Id_Dep = departamento.Id_Dep', $join_type);
    ...
    // funciones anidada para la tabla departamento 
    function get_foreing_Id_Dep($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('departamento.*,'.$this->table.'.*');
            $this->db->join('departamento', $this->table.'.Id_Dep = departamento.Id_Dep', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-10 01:50:12 */
/* www.margunsoft.com */