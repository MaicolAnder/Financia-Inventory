...
class Permiso_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'permiso',
        $elements = 'Id_Perm,Descripcion_Perm,Acceso_Perm,Controlador_Perm', 
        $find_id = 'Id_Perm',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'permiso.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('permiso/read/$1'),'Read')." | ".anchor(site_url('permiso/update/$1'),'Update')." | ".anchor(site_url('permiso/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('permiso/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('permiso/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('permiso/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
        if ($q != NULL) {
			$this->db->like('Id_Perm', $q);
			$this->db->or_like('Descripcion_Perm', $q);
			$this->db->or_like('Acceso_Perm', $q);
			$this->db->or_like('Controlador_Perm', $q);
		}
