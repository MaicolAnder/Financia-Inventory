...
class Contratos_model extends CI_Model
{
    ...
    function json(
        ...
        $links=[],
        $view = 'contratos',
        $elements = 'Id_Con,Numero_Con,FechaInicio_Con,FechaFin_Con,Valor_Con,Objeto_Con,Observacion_Con,Id_Emp,Id_ConTip,Id_ConEst,Primary_Usu', 
        $find_id = 'Id_Con',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'contratos.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('contratos/read/$1'),'Read')." | ".anchor(site_url('contratos/update/$1'),'Update')." | ".anchor(site_url('contratos/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('contratos/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('contratos/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('contratos/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
        if ($q != NULL) {
			$this->db->like('Id_Con', $q);
			$this->db->or_like('Numero_Con', $q);
			$this->db->or_like('FechaInicio_Con', $q);
			$this->db->or_like('FechaFin_Con', $q);
			$this->db->or_like('Valor_Con', $q);
			$this->db->or_like('Objeto_Con', $q);
			$this->db->or_like('Observacion_Con', $q);
			$this->db->or_like('Id_Emp', $q);
			$this->db->or_like('Id_ConTip', $q);
			$this->db->or_like('Id_ConEst', $q);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('contrato_estado.*');
            $this->db->select('contrato_tipo.*');
		} else{
            $this->db->select($elements);
        }
        $this->db->join('empresa', $this->table.'.Id_Emp = empresa.Id_Emp', $join_type);
        $this->db->join('contrato_estado', $this->table.'.Id_ConEst = contrato_estado.Id_ConEst', $join_type);
        $this->db->join('contrato_tipo', $this->table.'.Id_ConTip = contrato_tipo.Id_ConTip', $join_type);
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
    ...
    // funciones anidada para la tabla contrato_estado 
    function get_foreing_Id_ConEst($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('contrato_estado.*,'.$this->table.'.*');
            $this->db->join('contrato_estado', $this->table.'.Id_ConEst = contrato_estado.Id_ConEst', $join_type);
		} else{
            $this->db->select($elements);
        }
    ...
    // funciones anidada para la tabla contrato_tipo 
    function get_foreing_Id_ConTip($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('contrato_tipo.*,'.$this->table.'.*');
            $this->db->join('contrato_tipo', $this->table.'.Id_ConTip = contrato_tipo.Id_ConTip', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* Generated by Harviacode Codeigniter CRUD Generator 2020-05-10 01:49:54 */
/* www.margunsoft.com */