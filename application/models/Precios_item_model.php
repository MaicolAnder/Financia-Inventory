...
class Precios_item_model extends CI_Model
{

    public $table = 'precios_item';
    ...
    function json(
        ...
        $links=[],
        $view = 'precios_item',
        $elements = 'Id_ListPre,Id_Ite,PrecioVenta,Primary_Usu', 
        $find_id = 'Id_ListPre',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'precios_item.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('precios_item/read/$1'),'Read')." | ".anchor(site_url('precios_item/update/$1'),'Update')." | ".anchor(site_url('precios_item/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('precios_item/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('precios_item/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('precios_item/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function get_foreing_by_id($id, $elements='*', $join_type='LEFT')
    {
        ...
        if($elements=='*'){
            ...
            $this->db->select('lista_precios.*');
		} else{
            $this->db->select($elements);
        }
        $this->db->join('items', $this->table.'.Id_Ite = items.Id_Ite', $join_type);
        $this->db->join('lista_precios', $this->table.'.Id_ListPre = lista_precios.Id_ListPre', $join_type);
		$this->db->where($this->table.'.Primary_Usu', $this->session->userdata('Primary_Usu'));
    ...
    // funciones anidada para la tabla lista_precios 
    function get_foreing_Id_ListPre($elements='*', $where=[], $join_type='INNER', $order=[], $limit=[], $group=[], $having=[])
    {
        if($elements=='*'){ 
	
            $this->db->select('lista_precios.*,'.$this->table.'.*');
            $this->db->join('lista_precios', $this->table.'.Id_ListPre = lista_precios.Id_ListPre', $join_type);
		} else{
            $this->db->select($elements);
        }
...
/* End of file Precios_item_model.php */
/* Location: ./application/models/Precios_item_model.php */
