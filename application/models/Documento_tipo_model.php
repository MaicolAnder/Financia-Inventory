...
class Documento_tipo_model extends CI_Model
{

    public $table = 'documento_tipo';
    public $id = 'Id_DocTip';
    public $order = 'DESC';
    ...
    function json(
        ...
        $links=[],
        $view = 'documento_tipo',
        $elements = 'Id_DocTip,Nombre_DocTip,Estado_DocTip,FechaRegistro_DocTip', 
        $find_id = 'Id_DocTip',
        $join=[]) {
        ...
        //$this->datatables->join('table2', 'documento_tipo.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('documento_tipo/read/$1'),'Read')." | ".anchor(site_url('documento_tipo/update/$1'),'Update')." | ".anchor(site_url('documento_tipo/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), $find_id);
        // Métodos estaticos para link en respuesta de datatables
        // $this->datatables->add_column('ver', anchor(site_url('documento_tipo/read/$1'),'<i class="fas fa-eye"></i>',array('title'=>'Ver registro','class' => 'btn btn-info')),$find_id);
        // $this->datatables->add_column('actualizar', anchor(site_url('documento_tipo/update/$1'),'<i class="fas fa-pencil-alt"></i>',array('title'=>'Actualizar','class' => 'btn btn-primary2')),$find_id);
        // $this->datatables->add_column('eliminar', anchor(site_url('documento_tipo/delete/$1'),'<i class="fas fa-trash-alt"></i>',array('title'=>'Eliminar','class' => 'btn btn-danger', 'onclick'=>'javasciprt: return confirm(\'¿Esta seguro?\')')),$find_id);
    ...
    function total_rows($q = NULL) {
        if ($q != NULL) {
			$this->db->like('Id_DocTip', $q);
			$this->db->or_like('Nombre_DocTip', $q);
			$this->db->or_like('Estado_DocTip', $q);
			$this->db->or_like('FechaRegistro_DocTip', $q);
		}
