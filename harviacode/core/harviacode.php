...
class Harviacode
{
    ...
    function fk_field($column_name)
    {
        $query = "SELECT TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=? AND COLUMN_NAME = ? AND COLUMN_KEY = 'PRI'";
        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (fk_field)");
        $stmt->bind_param('ss', $this->database, $column_name);
        $stmt->bind_result($fk_table_name, $fk_column_name);
        $stmt->execute();
        $stmt->fetch();
    ...
    function all_fk_field($table){
		...
		$stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (not_primary_fields)");
		$stmt->bind_param('ss', $this->database, $table);
		$stmt->bind_result($column_name, $fk_Table, $fk_column);
        $stmt->execute();
        $fields = array();
        while ($stmt->fetch()) {
            $fields[] = array('column_name' => $column_name, 'fk_Table' => $fk_Table, 'fk_column' => $fk_column);
        }
